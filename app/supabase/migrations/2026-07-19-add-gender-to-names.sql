-- Adds a gender column to Names and teaches get_leaderboard to filter by it.
-- The app already sends these values and the optional p_genders parameter.
begin;

create type public.name_gender as enum (
  'always_male',
  'mostly_male',
  'neutral',
  'mostly_female',
...
alter table public."Names"
  add column gender public.name_gender null;
...
begin
  ...
  where p_genders is null
    or n.gender::text = any(p_genders)
  order by
